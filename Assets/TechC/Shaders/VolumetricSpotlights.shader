Shader "Effect/VolumetricSpotlights"
{
	Properties
	{
	}
	SubShader
	{
		Pass
		{
			...
			float4 frag(Varyings IN) : SV_Target
			{
				...
				float exitWS = min(backFaceWS, opaqueWS);
				clip(exitWS - enterWS);
				...
				int2 stepEnterExit = (int2)clamp(ceil(inverseLerp(boundsNearFarWS.x, boundsNearFarWS.y, float2(enterWS, exitWS)) * MAX_STEPS), 0, MAX_STEPS);
				...
				for (int i = stepEnterExit.x; i < stepEnterExit.y; i++)
				{
					...
					for (uint lightIndex = 0u; lightIndex < (uint)_LightsCount; lightIndex++)
					{
